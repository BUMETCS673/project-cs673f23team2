...
describe('API Test for /getKeywordData', () => {
    it('should get keyword data', () => {
      // Define test data
      ...
      cy.request({
        method: 'GET',
        url: 'http://localhost:5000/getKeywordData',
        qs: {
          userId: userId,
        },
      }).then((response) => {
        ...
        if (response.body.firstClickData) {
          // Check if the required fields are present in the keyword data
